...
1. Clang — это компилятор C/C++/Objective-C, часть проекта LLVM.
Роль: Преобразует исходный код в AST; Обрабатывает макросы и директивы препроцессора; Генерирует LLVM IR для дальнейшей оптимизации.

2. LLVM — инфраструктура для создания компиляторов.
Использование: Предоставляет универсальное промежуточное представление (IR); Оптимизирует код независимо от языка/архитектуры; Позволяет создавать JIT-компиляторы.

3. AST (Абстрактное синтаксическое дерево)

— Это структура данных, отражающая иерархию исходного кода с сохранением его синтаксиса (условия, циклы, объявления).

— Зависит от языка (разное для C++, Python и т.д.).

— Используется для семантического анализа (проверка типов, областей видимости).

LLVM IR (Промежуточное представление)

— Это низкоуровневый ассемблероподобный код в SSA-форме (каждая переменная назначается один раз).

— Не зависит от языка (единый для всех языков, поддерживаемых LLVM).

— Оптимизируется (inlining, loop unroll) перед генерацией машинного кода.

4. Разделяет фронтенд (анализ кода) и бэкенд (генерация машинного кода); Позволяет применять кроссплатформенные оптимизации; Упрощает поддержку новых языков/архитектур.

5.
Что делает:

- Выделяет память в стеке функции для переменных

- Возвращает указатель на выделенную область

Зачем используется:

- Для временных переменных до оптимизации mem2reg

- Хранение адресуемых значений (например, массивов)

6. Цель оптимизации — улучшить качество сгенерированного кода: уменьшение количества инструкций, повышение эффективности; Память — снижение потребления памяти, устранение ненужных аллокаций.

7. SSA (Static Single Assignment) — это форма представления кода, в которой каждая переменная присваивается только один раз.

8. CFG (Control Flow Graph) — это ориентированный граф, в котором: вершины — базовые блоки (последовательности инструкций без ветвлений внутри), рёбра — возможные переходы управления между блоками.

9. В LLVM IR арифметика представляется низкоуровневыми инструкциями, тип привязан явно.

10. Легко анализировать отдельно (модульность). Подходят для локальных оптимизаций: инлайнинг, удаление неиспользуемых переменных и блоков.

11. Если функция вызывается один раз и является очень короткой (например, состоит из одной-двух операций), LLVM встраивает ее тело в место вызова, удаляет функцию из IR.

12.
- IR унифицирует разные языки (C, C++, Rust) в один промежуточный формат, упрощая анализ.

- Нет зависимости от особенностей синтаксиса (скобки, макросы и т.д.).

- Легко применять оптимизации.
  
- IR не привязан к архитектуре (x86/ARM), поэтому оптимизации работают везде.
  
- Граф потока управления строится автоматически.
  
- В IR явно видны: работа с памятью (load/store), вызовы функций (call), типы данных (i32, double).
...
3.	Передаёт real и imag как аргументы.

5.	Конструктор сохраняет значения в структуру.
...
5.	Вызывает вспомогательную функцию, которая вызывает cabs(re, im).
...
При передаче в функцию printPoint, структура передаётся по значению, т.е. создаётся её копия.

Поскольку структура маленькая, она встраивается напрямую в регистры или стек.
...
IR опускает "синтаксический сахар" и фокусируется на инструкциях, близких к ассемблеру.

Прототип в IR либо объявляется через declare, либо заменяется на define, если есть тело функции.
